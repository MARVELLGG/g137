...
void GrowtopiaBot::onLoginRequested() {
    ...
    // Data login bot disimpan dalam sebuah vector
    ...
    // Kirimkan setiap paket login ke masing-masing peer
    for (size_t i = 0; i < peer.size(); ++i) {
        cout << "Sending login packet to peer " << i + 1 << "...\n";
        SendPacket(2, packets[i], peers[i]);
    }
